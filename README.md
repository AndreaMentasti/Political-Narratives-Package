...
- **1** (For beginner python users) The first step is to download Anaconda and Anaconda Prompt at the [link](https://www.anaconda.com/docs/getting-started/anaconda/install).
- **2** Open Anaconda Prompt and set the ````OPENAI_API_KEY```` as an environmental variable in your current environment (likely ````base````):
  ````bash
