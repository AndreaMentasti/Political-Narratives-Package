...
def render_step(step: int):
    ...
    if step == 3:
            ...
            question_card(
                ...
                how_to=[
                    "As said above, each **character** should be seen as a category and not be mistaken with *actors*. The first step in defining characters is to read through your data. Before the LLM can encode narratives at scale, you need to make informed, grounded decisions based on what a human reader would plausibly identify. ",
                    "Identify relevant characters for the topic. You can base your selection on prior literature, your own research focus, or take a more data-driven approach. Characters can be identified manually, by reading your texts and noting recurring entities, or with the help of automated tools, though these are optional. ",
                    "Traditional tools such as word clouds or topic models can help. You may also use entity recognition with the more advanced [RELATIO package](https://github.com/relatio-nlp/relatio). However, all these methods eventually require dimensionality reduction: from the full set of entities they surface, you must decide how to group and narrow them down to a manageable, meaningful set of characters. Unlike topic modeling or RELATIO, in our framework, this aggregation step happens before the large-scale annotation: it is a necessary act of simplification that defines what the LLM will look for. ",
    ...
    if step == 4:
        ...
        question_card(
            ...
            blurb_md=(
                "This step is crucial for achieving **accurate classification**. To ensure that the LLM assigns the correct roles to each character, you must provide **clear and precise instructions** in the form of a **SYSTEM MESSAGE**, formatted as a JSON file. "
                "Your SYSTEM MESSAGE should specify three components: first, the task the LLM must perform on each text snippet; "
                "second, the list of characters to identify, each with a brief description and, if useful, clarifying examples; and third, the set of possible roles they can assume — **Hero**, **Villain**, or **Victim**.\n\n "
                "Below, we provide key guidance for constructing the SYSTEM MESSAGE. A ready-to-use example prompt is available in the "
                "[GitHub repository](https://github.com/AndreaMentasti/Political-Narratives-Package/tree/main) and can be easily adapted by following these instructions.\n\n "
                "To further improve data quality, we also include an optional **relevance classification** prompt. This step helps assess how well the selected text snippets relate to your topic. "
                "Keyword-based filtering (used in Step 2) can be noisy and often includes texts that are not truly relevant. Such noise reduces data quality, making both classification and later analysis more difficult. "
                "The relevance classification prompt helps filter these texts out, saving both cost and time. If you choose to apply it, first run the **relevance classification** (Stage 1), and then apply the **character-role classification** (Stage 2) only to those texts labeled as relevant.\n\n"
                "##### Step-by-Step Instructions:\n"
                "1) Access the [GitHub repository](https://github.com/AndreaMentasti/Political-Narratives-Package/tree/main) and download the `system_message_stage1` file for relevance classification and the `system_message_stage2` file for character-role classification from the `codes and prompts/` folder.\n\n "
                "2) For the relevance classification (`system_message_stage1`), adjust the instructions in the SYSTEM MESSAGE to reflect your specific topic. No other changes are required for this prompt.\n\n"
                "3) For the character-role classification (`system_message_stage2`), modify the instructions, update the list of characters, and provide their descriptions.\n\n"
                ...
                "This example comes from our paper, where we focused on assessments by US citizens. Narratives are always context-dependent, and LLMs can capture this to some extent. "
                "We strongly recommend adapting this part to your target population. For instance, if your study focuses on India, you might write *You are an average Indian citizen*. "
                "If you analyze five European countries, you could either run separate prompts for each (to capture cultural nuances) or a single prompt reflecting an *average European perspective*. "
                "Ultimately, this choice depends on your research goals, but remember that context matters. Modify these few instructions to match your specific task. "
                "If you are working on character-role classification, continue with the next steps; if you are only running relevance classification, you can proceed directly to Step 5.\n\n "
                "4) In `system_message_stage2`, update the names and descriptions of your characters. In our example, we included ten characters, labeled with keys from `a` to `j`. "
                "Each character has a short description and an example. Replace these with your own topic-specific characters and descriptions.\n\n"
                "5) After each description, specify the key where the result will be stored. The alphabetical keys should be lowercase, starting from `a` and continuing sequentially (e.g., `a–f` for six characters or `a–h` for eight). "
                "For instance, the classification output for the first character will be stored in column `a`, for the second in column `b`, and so on. "
                "These keys simply define how the LLM stores results in the output and can be modified without any knowledge of JSON syntax.\n\n"
            ),
        )
            ...
            blurb_md=r"""
              ...
              def build_user_content(article_group):
               ...
               ```bash
                ...
                Temperature is a parameter that controls the “creativity” or randomness of the text generated by the GPT model. A higher temperature (e.g., 0.7) results in more diverse and creative output, while a lower temperature (e.g., 0.2) makes the output more deterministic and focused.  
                ...
                Lastly, the user can also decide wich GPT model to use. This code allows for **gpt-4o**, **gpt-4o-mini**, **gpt-4.1**, and **gpt-4.1-mini**."""
