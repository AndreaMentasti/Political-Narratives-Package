...
main = r"C:\Users\AndreaMentasti\Dropbox\replication_github"
...
if resume_run_folder:
    ...
    run_partial_results_folder = os.path.join(output_data_folder, f"zs_run_{unique_run_id}")
    run_final_results_folder = os.path.join(final_data_folder, f"zs_run_{unique_run_id}")
...
csv_path = os.path.join(main, "data", "output", "your_data_stage1.csv")
path_df = os.path.join(main, "data", "output", "your_data_stage1.feather")
...
def build_user_content(article_group):
    ...
    user_content = (
        ...
        "Each object MUST include keys: id, r (r in {0,1,2,3}). "
            ...
            try:
                ...
                for entry in json_list:
                    ...
                    flattened_entry = {
                        ...
                        "id": entry_id,
                        # default to 0 (irrelevant) if missing
                        "r": entry.get("r", 0)
                    }
...
def run_classification_in_chunks(chunk_size=100):
    ...
    articles = [{"id": row["id"], "text": row["text"]} for _, row in df.iterrows()]
...
def merge_partial_results_into_final():
    ...
    if df_list:
        ...
        final_csv_path = os.path.join(run_final_results_folder, "flattened_results_all_zs.csv")
